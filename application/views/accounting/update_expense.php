					...
					<div class="row-fluid">
						...
						<!--check any alert message or not -->
						 ...
						 <!--Print Wrong Alert Message: -->		
							...
							<?php endif; ?>	
						<!--------------End of Message---------------------------------->
							<?php echo form_open_multipart(base_url().'accounting/edit_expense','class="form-horizontal"'); ?>
									...
									<div class="controls">
										<?php
					 					$title = array(
											...
											'value'			=> (@$this->input->post('amount')) ? 	@$this->input->post('amount') : $row->amount
										);
									...
									<div class="controls">
										<?php
					 					$title = array(
											...
											'value'			=> (@$this->input->post('title')) ? @$this->input->post('title') : $row->title,	
										);
										echo form_input($title);
								...
								<div class="control-group">
									...
									<div class="controls">
										<textarea name="details" rows="5" id="form-field-11" class="autosize-transition span3"><?php echo  (@$this->input->post('datails')) ? @$this->input->post('details') : $row->details; ?></textarea>
									</div>
								</div>
								<div class="control-group">
									...
									<div class="controls">
										...
										endforeach;
									 	echo form_dropdown('expenceCategoryId', $cate_list,(@$this->input->post('expenceCategoryId')) ? @$this->input->post('expenceCategoryId') : $row->expenceCategoryId);
									...
									<div class="controls">
										<?php
					 					$mediam = array(
											...
											'id'			=>'form-field-1',
											'placeholder'	=>'Expense amount',
											'value'			=> (@$this->input->post('mediam')) ? 	@$this->input->post('mediam') : $row->mediam
										);
								 ...
								 <div class="control-group">
									<label class="control-label" for="form-field-1">Date Of Expense</label>
									<div class="controls">
										<input class="input-append date-picker" name="date" placeholder="Chossse a date" id="id-date-picker-1" type="text" data-date-format="dd-mm-yyyy" value="<?php echo (@$this->input->post('date')) ? @$this->input->post('date') : date('d-m-Y', strtotime($row->date)) ;?>"/>
										<span class="add-on"> 
